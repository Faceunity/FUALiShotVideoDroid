...
public class STBeautifyNative {
     ...
     * 初始化beutify引擎
     *
     * @param width  输入图片的宽度(以像素为单位)
     * @param height 输入图片的高度(以像素为单位)
     ...
     * @param arrayFacesIn 输入需要美颜的人脸106点数组；如果为NULL，不执行大眼瘦脸
     * @param outFormat    输出图片的类型,支持NV21,BGR,BGRA,NV12,RGBA等。比如STCommon.ST_PIX_FMT_NV21
     * @param arrayFacesOut 输出美颜后的人脸106点数组，需要由用户分配内存，必须与输入的人脸106点数组大小相同；如果为NULL，不输出美颜后的人脸106点数组
     * @return 成功返回0，错误返回其他，参考STUtils.ResultCode
     */
    public native int processBufferInGLContext(byte[] pInputImage, int inFormat, int outputWidth, int outputHeight, STMobile106[] arrayFacesIn, byte[] pOutImage, int outFormat, STMobile106[] arrayFacesOut);
     ...
     * @return 成功返回0，错误返回其他，参考STUtils.ResultCode
     */
    public native int processBufferNotInGLContext(byte[] pInputImage, int inFormat, int outputWidth, int outputHeight, STMobile106[] arrayFacesIn, byte[] pOutImage, int outFormat, STMobile106[] arrayFacesOut);
     ...
     * @return 成功返回0，错误返回其他，参考STUtils.ResultCode
     */
    public native int processTexture(int textureIn, int outputWidth, int outputHeight, STMobile106[] arrayFacesIn, int textureOut, STMobile106[] arrayFacesOut);
     ...
     * @return 成功返回0，错误返回其他，参考STUtils.ResultCode
     */
    public native int processTextureAndOutputTexture(int textureIn, int outputWidth, int outputHeight, STMobile106[] arrayFacesIn, int textureOut, byte[] outputBuf, int format, STMobile106[] arrayFacesOut);
